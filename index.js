...
const SNAKE_BODY = [SnakeBody, SnakeTail]
...
function updateSnakePosition() {
    ...
    for(var i = 0; i < SNAKE_BODY.length; i++) {
        ...
        if(intersectOnXZProjection(SnakeHead.getPosition(), currPosition)) {
...
function animateSnakeRotation(dTheta, timerName) {
    var oldRotY = SnakeHead.getRotationY();
    var dtheta = 0;
    ...
    Hatch.createTimer(timerName, 1, function () {
        ...
        SnakeHead.setRotationY(oldRotY + dtheta);
    });
}
...
function growSnakeAtNeck() {
    Hatch.cloneObject(SnakeBody, function(newBodySegment) {
